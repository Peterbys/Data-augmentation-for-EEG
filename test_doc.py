...
data_epsilon = generate_data_bem(X,data.shape[0],data.shape[1],b,src,trans,subjects_dir,raw_fname,conductivity,K)
data_epsilon =  np.concatenate((data[None,:,:],data_epsilon))
#%%
...
X = np.linalg.pinv(gain) @ data
data_epsilon_sphere = generate_data_bem(X,data.shape[0],data.shape[1],b,src_sphere,trans,subjects_dir,raw_fname,conductivity,K)
data_epsilon_sphere = np.concatenate((data[None,:,:],data_epsilon_sphere))
...
#%%
tester_sphere = np.reshape(data_epsilon,(data_epsilon.shape[0]*data_epsilon.shape[1],data_epsilon.shape[2]) )
#%%
