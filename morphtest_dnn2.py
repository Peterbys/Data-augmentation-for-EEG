...
class binaryClassification(nn.Module):
    def __init__(self,l1,l2,l3,l4):
        ...
        self.batchnorm3 = nn.BatchNorm1d(l4)
...
#model = binaryClassification(X_train.shape[1],int(X_train.shape[1]*1.5),int(X_train.shape[1]*1.5),int(X_train.shape[1]))
model = binaryClassification(X_train.shape[1],int(X_train.shape[1]/2),int(X_train.shape[1]/4),int(X_train.shape[1]/8))
model.to(device)
print(model)
...
optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE,weight_decay = 1.2)
...
for e in range(e_end, EPOCHS):
    ...
    with torch.no_grad():
        for X_batch_test,y_batch_test in test_loader_other:
            ...
            acc_test_out[e-1] += binary_acc(y_pred_tag, y_batch_test.unsqueeze(1)).item()/ len(y_test_other)
    model.eval()
    with torch.no_grad():
        for X_batch_test,y_batch_test in test_loader:
            ...
            y_test_pred = torch.sigmoid(y_test_pred)
            y_pred_tag = torch.round(y_test_pred)
            acc_test_in[e-1] += binary_acc(y_pred_tag, y_batch_test.unsqueeze(1)).item()/ len(y_test)
